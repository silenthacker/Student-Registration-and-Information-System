...
{
    public partial class DisableTeacherForm : Form
    {
        Variables variables;
        ...
        private void DisplayUsersListInHumanReadableFormat()
        {
            try
            {
                string sqlquery1 = "SELECT * FROM [Tbl.Users] WHERE [ACCOUNT TYPE] = 'School Teacher'";
        ...
        private void UserIDTextbox_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            else if (e.KeyCode == Keys.Enter)
            {
                ...
                try
                {
                    this.opacityform = new OpacityForm();
                    this.darkeropacityform = new DarkerOpacityForm();
                    ...
                    else
                    {
                        string sqlquery1 = "SELECT COUNT(*) FROM [Tbl.Users] WHERE [USER ID] = '" + UserIDTextbox.Text.Trim() +
                            "' AND [ACCOUNT TYPE] = 'School Teacher'";
                        sqldataadapter = new SqlDataAdapter(sqlquery1, sqlconnection);
                        DataTable datatable = new DataTable();
                        sqldataadapter.Fill(datatable);
                        ...
                        if (datatable.Rows[0][0].ToString() == "1")
                        {
                            ...
                            else if (isActive.Equals("Disabled"))
                            {
                                notificationwindow.CaptionText = "MESSAGE CONTENT";
                                notificationwindow.MsgImage.Image = Properties.Resources.check;
                                notificationwindow.MessageText = "THIS TEACHER ACCOUNT IS\nALREADY DISABLED !";
                ...
                {
                    opacityform.Show();
                    MessageBox.Show(exception.Message.ToString(), "Disable Teacher Form Exception 2",
                         MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void DisableTeacherForm_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Escape)
            {
                DialogResult = DialogResult.OK;
